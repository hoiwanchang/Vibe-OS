...
# ============================================================================
# firstboot.sh — 首次启动引导
# 由 vibeos-firstboot.service（Type=oneshot, ConditionPathExists）触发一次
# 职责：生成 API token、获取 Tailscale/局域网地址、输出引导信息、标记完成
# ============================================================================
set -euo pipefail

ENV_FILE="/etc/vibeos.env"
SECRETS_DIR="/data/vibeos/secrets"
DONE_FLAG="/var/lib/vibeos/.firstboot-done"
CONSOLE_PORT="${VIBEOS_PORT:-3000}"
...
main() {
  ...
  if ! grep -q '^VIBEOS_API_TOKEN=' "$ENV_FILE" 2>/dev/null; then
    local token
    token="$(openssl rand -hex 32)"
    echo "VIBEOS_API_TOKEN=${token}" >> "$ENV_FILE"
    log "已生成 API token"
  fi

  # [安全加固] 多盘检测警告：preseed early_command 写入的标记
  local disk_warning=""
  if [[ -f /var/lib/vibeos/.disk-warning ]]; then
    disk_warning="$(cat /var/lib/vibeos/.disk-warning)"
  ...
  local install_error=""
  if [[ -f /var/lib/vibeos/.install-error ]]; then
    install_error="$(cat /var/lib/vibeos/.install-error)"
    log "❌ 检测到安装时运行时部署失败: $install_error"
    log "   请手动执行 /opt/vibeos/install-runtime.sh 或联系支持"
  fi
  ...
  cat > /data/vibeos/firstboot-info.json <<EOF
{
  "hostname": "$(hostname)",
...
EOF
  chmod 644 /data/vibeos/firstboot-info.json

  # 生成 API token 后重启 Web 控制台（确保服务以新 token 启动）
  systemctl restart vibeos-web-console.service 2>/dev/null || true

  log "======================================================"
  log " Vibe OS 安装完成，Web 控制台引导页："
